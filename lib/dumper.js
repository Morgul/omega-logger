...
function Dumper(target, depthOrOptions)
{
    ...
    {
        for(var key in depthOrOptions)
        {
            this.persistentInspectOpts[key] = depthOrOptions[key];
        } // end for
    }
    else
    {
    ...
    this.indent = (depthOrOptions || {}).indent || '';
} // end Dumper
...
{
    return util.inspect(this.target, this.inspectOpts).replace(newlineRE, '\n' + this.indent);
}; // end Dumper#toString
